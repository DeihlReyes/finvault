...
import { useDashboardData } from "@/lib/hooks/use-db-queries";
import { formatCurrency } from "@/lib/utils";
import { xpProgressToNextLevel } from "@/types/gamification";
import { CHALLENGE_TARGET } from "@/lib/challenges/monthly";
...
export default function DashboardPage() {
  ...
  const { income: monthIncome, expenses: monthExpenses } = monthlyAggs;
  const savings = monthIncome - monthExpenses;
  const sparklineData = snapshots.map((s) => ({ value: Number(s.totalValue) }));
  const { level, currentLevelXP, nextLevelXP, progress } =
    xpProgressToNextLevel(user.totalXP);

  // Budget health — top 3 by percentage
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 mx-auto">
      ...
      {budgetsWithSpend.length > 0 && (
        <BudgetHealthStrip budgets={budgetsWithSpend} currency={user.currency} />
      )}
